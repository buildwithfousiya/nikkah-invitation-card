...
// ── Wedding date target ────────────────────
const WEDDING_DATE = new Date('2026-07-25T10:30:00+05:30');
...
async function downloadWeddingCard(btn) {
  ...
  const FILENAME = 'Haseeb-Naila-WeddingCard.png';
  ...
  try {
    ...
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({
        files: [file],
        title: 'Haseeb & Naila — Wedding Card',
        text: 'You\'re invited! ✨'
      });
    ...
    // If fetch/blob or share fails (common in local file protocol or CORS), do a direct link click download
    const a = document.createElement('a');
    a.href = CARD_URL;
    a.download = FILENAME;
...
document.querySelectorAll('.download-btn:not(.disabled)').forEach(btn => {
  btn.addEventListener('click', (e) => {
    // Only intercept with JavaScript if native mobile sharing is available.
    // Otherwise, let the anchor tag's default download/open behavior run natively.
    if (navigator.share && navigator.canShare) {
      e.preventDefault();
      downloadWeddingCard(btn);
    }
  });
});
